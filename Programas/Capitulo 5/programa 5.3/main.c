...
Y = *IX;              /* Y toma e valor de Z[2], ahora vale 6. */
...
X = *(IX + 1);        /* X se modifica. el apuntador IX accede temporalmente a Z[3], por lo tanto X roma este valor (8). Obsrva que IX no se reasigna */
Y= *IX;               /* Y se modifica, toma le valor de Z[2] (12). */
printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2]= %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
...
IX = IX + 4;          /* IX se modifica. Observe que el apuntador se mueve 4 posiciones y cae en una direccion afuera del arreglo. esto pcasionara un error. */
Y = IX;               /* Y se modifica, toma el valor (basura) de una celda incorrecta. Es un error que no señla el compilador del lenguaje C. */
printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2]= %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

IX = &X;              /* IX apunta a l variable entera X. */
IX = IX +1;           /* IX se mueve una posicion y cae ebn una celda incorrecta. */
