...
int main(void)
{
    FILE *ar;
    FILE *ap;
    ar = fopen("C:\\Users\\sv988\\Desktop\\programacion 2\\Tarea #6\\Capitulo #9\\programa RP9.5\\bin\\r", "r");        /* Se abre el archivo arc.txt para lectura. */
...
void cambia(FILE *ap1, FILE *ap2)
{
    ...
    while (!feof(ap1))
    {
        ...
        while (cad2 != NULL)
        {
            ...
            if (k)
            {
                strncpy(aux, cad1, k); /* Se copia la subcadena de k caracteres de cad1 a aux —desde el
